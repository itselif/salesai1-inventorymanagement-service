...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("inventoryManagementShareToken");
  await elasticIndexer.indexData(data);
};

const updateInventoryManagementShareTokenById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await InventoryManagementShareToken.findOne({
    ...
    const [rowsCount, [dbDoc]] = await InventoryManagementShareToken.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateInventoryManagementShareTokenById;
