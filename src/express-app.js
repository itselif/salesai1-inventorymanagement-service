...
//read tenant specific parameeters to the request root
app.use((req, res, next) => {
  const pathParts = req.path.split("/").filter(Boolean);
  if (pathParts.length > 0 && pathParts[0].startsWith("$")) {
    req.pathTenantCodename = pathParts[0].substring(1);
...
const exposedHeaders = [
  "salesai1-access-token",
  "salesai1-refresh-token",
  "set-cookie",
];
if (!corsOrigin) {
  app.use(
...
app.get("/getPostmanCollection", (req, res) => {
  const filePath = path.join(
    __dirname,
    "salesai-inventoryManagement-postman-collection.json",
  );
  console.log("Downloading Postman Collection from:", filePath);
  res.setHeader("Content-Type", "application/json");
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="salesai-inventoryManagement-postman-collection.json"',
  );
